 ...
 * @author zacht
 */
public class highrounds extends HttpServlet {
    ...
    @Override
        ...
        try {
            ...
            //Creating the table
            String sqlCreate = "CREATE TABLE IF NOT EXISTS highroundsData (" +
                   "`formNumber` INT AUTO_INCREMENT PRIMARY KEY, " +
                   "`map` CHAR(40), `gamertag` CHAR(40), `time` CHAR(40), " +
                   "`round` CHAR(40), `platform` CHAR(40));";
        ...
        try {
            //String query = "INSERT INTO highroundsData VALUES (?,?,?,?,?,?,?,?,?)";
            //A String variable that will hold the sql code to insert values
            String query = "INSERT INTO highroundsData (map, gamertag, time, round, platform) VALUES (?, ?, ?, ?, ?)";
            ...
            try ( PrintWriter out = response.getWriter()) {
                ...
                out.println("<html>");
                out.println("<head>");
                out.println("<title>Servlet highrounds</title>");
                out.println("</head>");
                out.println("<body>");
        ...
        String sql = "UPDATE highroundsData SET map=?, gamertag=?, time=?, round=?, platform=? WHERE formNumber=?";
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, map);
            pstmt.setString(2, gamertag);
            pstmt.setString(3, time);
            pstmt.setString(4, round);
        ...
        try {
            String sql = "SELECT * FROM highroundsData WHERE formNumber = ?";
            ...
            if (rs.next()) {
                ...
                out.println("<p>Round: " + rs.getString("round") + "</p>");
    ...
    private void deleteForm(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int formNumber = Integer.parseInt(request.getParameter("formNumber"));
        String sql = "DELETE FROM highroundsData WHERE formNumber = ?";
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
